...
def main():
    ...
    if args.mgi:
        ...
        if "fastq_list" in df.columns and df["fastq_list"].notna().any():
            df_fastqlist = (
                df[df["fastq_list"].notna()].copy().drop(columns=["read1", "read2", "cram", "bam"], errors="ignore")
            )
        ...
        if "cram" in df.columns and df["cram"].notna().any():
            df_cram = (
                df[df["cram"].notna()].copy().drop(columns=["read1", "read2", "fastq_list", "bam"], errors="ignore")
            )
