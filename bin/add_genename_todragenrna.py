...
def main():
    ...
    parser.add_argument(
        "-t", "--transcripts", action="store_true", default=False, help="Convert transcripts instead of genes"
    )
    ...
    if args.transcripts:
        transcripts = pd.read_csv(args.database, sep="\t")[
        ...
        outdf.loc[outdf["transcript_name"].isna(), "transcript_name"] = outdf.loc[
            outdf["transcript_name"].isna(), "transcript_id"
        ]
        outdf.rename(
            columns={"Name": "Id", "transcript_name": "Name", "gene_id": "GeneId", "gene_name": "GeneName"},
            inplace=True,
        )
        outdf["Id Name GeneId GeneName Length EffectiveLength TPM NumReads".split(" ")].to_csv(
