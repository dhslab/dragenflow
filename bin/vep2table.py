...
def pos2codon(exonstart, exonend, cpos, pos, strand):
    if pos <= exonend and pos >= exonstart:
        if strand == "+":
            return int((pos - exonstart + cpos) / 3 + 0.99)

        elif strand == "-":
            return int((exonend - pos + cpos) / 3 + 0.99)
...
def parse_small_variants(vcffile, individual=[0]):
    ...
    variants_columns += [f"{sample}_{suffix}" for sample in sample_names for suffix in ["coverage", "altreads", "vaf"]]
...
def parse_svs(svvcffile, individual=[0]):
    ...
    for variant in svvcf:
        ...
        if variant.INFO["SVTYPE"] == "BND" or variant.INFO["SVTYPE"] == "INV":
            ...
            bands = variant.INFO.get("Cytobands").split("&")
            bandstring = "None"
            genestring = None
            genedetail = None
            total_genes = "None"
            ...
            if variant.INFO.get("CSQ"):
                ...
                vepCsq = vepToTable(variant.INFO.get("CSQ"), svvcf.get_header_type("CSQ"))

                total_genes = f"{len(vepCsq['SYMBOL'].unique())} genes"

                # get banks in Cytobands is in vepCsq
                bandstring = bands[0]
                if len(bands) > 1:
                    bandstring = bands[0] + bands[-1]

                vepCsq = vepCsq.sort_values(by=["PICK"], ascending=[False]).drop_duplicates(
                    subset="SYMBOL", keep="first"
                )
                vepCsq["GeneEffect"] = vepCsq.apply(lambda r: vepGeneEffect(r), axis=1)
                vepCsq["GeneImpact"] = vepCsq["Consequence"].apply(
                    lambda r: int(len(set(r.split("&")) & set(nonSynon)) > 0)
                )
                ...
                if len(vepCsq) > 10:
                    ...
                    genedetail = (
                        vartype.lower()
                        + "["
                        + ",".join(vepCsq.apply(lambda x: x["SYMBOL"] + x["GeneEffect"], axis=1).tolist())
                        + "]"
                    )
    ...
    for v in passedvars.items():
        ...
        if variant.INFO.get("CSQ"):
            ...
            bands1 = variant.INFO.get("Cytobands").split("&")[0]
            ...
            gene1Df = gene1Df.sort_values(
                by=["DISTANCE", "GeneEffect"], ascending=[True, False], na_position="last"
            ).fillna("")

            # get gene info for MATE
            vepCsq = vepToTable(mate.INFO["CSQ"], svvcf.get_header_type("CSQ"))

            bands2 = variant.INFO.get("Cytobands").split("&")[0]

            vepCsq = vepCsq.sort_values(by=["DISTANCE", "PICK"], ascending=[True, False]).drop_duplicates(
                subset="SYMBOL", keep="first"
            )
            vepCsq["GeneEffect"] = vepCsq.apply(lambda r: vepGeneEffect(r), axis=1)
            ...
            if (
                (
                    ...
                    and chr1.find("Y") == -1
                    and chr2.find("Y") == -1
                    and int(chr1.replace("chr", "")) < int(chr2.replace("chr", ""))
                )
                or chr1.find("X") > -1
                or chr1.find("Y") > -1
            ):
...
parser.add_argument(
    ...
    type=int,
    nargs="+",
    help="Individual in the VCF to return format fields",
)
...
if args.svvcf is not None:
    ind = args.individual
    svs = parse_svs(args.svvcf, individual=ind[0])
