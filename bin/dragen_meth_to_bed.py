...
def process_file(input_file, output_file):
    ...
    if output_file and output_file.endswith(".gz"):
        sorted_df.to_csv(
            output_file,
            sep="\t",
            columns=["Chromosome", "Start", "End", "ratio", "total"],
            header=False,
            index=False,
            compression="gzip",
        )
        ...
        sorted_df.to_csv(
            output_file, sep="\t", columns=["Chromosome", "Start", "End", "ratio", "total"], header=False, index=False
        )
    else:
        sorted_df.to_csv(
            sys.stdout, sep="\t", columns=["Chromosome", "Start", "End", "ratio", "total"], header=False, index=False
        )
