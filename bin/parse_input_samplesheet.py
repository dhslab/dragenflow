...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--output_dir", "-o", help="Directory to save the output CSV files")
...
def save_df(dataframe: pd.DataFrame, output_dir: str, filename: str) -> None:
    ...
    if not dataframe.empty:
        dataframe.loc[:, "id"] = dataframe["id"].apply(clean_sample_name)
...
def process_input(input_file: str, output_dir: str) -> None:
    ...
    if "dragen_path" in df.columns:
        ...
        analysis_df = df.dropna(subset=[col for col in analysis_cols if col in df], thresh=1)
        ...
        alignment_df = df.dropna(subset=[col for col in alignment_cols if col in df], thresh=1)
